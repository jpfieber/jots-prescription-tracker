import { App, Editor, MarkdownView, Modal, Notice, Plugin, PluginSettingTab, Setting, TFile, TFolder, TextComponent, AbstractInputSuggest, prepareFuzzySearch, SearchResult, normalizePath } from 'obsidian';
...
class PrescriptionModal extends Modal {
    ...
    async onOpen() {
        ...
        new Setting(contentEl)
            .setName('Drug Label')
            .setDesc('Select a file from the +Inbox folder for the drug label')
            .addDropdown(dropdown => {
                // Add empty option
                ...
                if (inboxFolder instanceof TFolder) {
                    const files = inboxFolder.children.filter(file => file instanceof TFile);
                    files.forEach(file => {
...
class PrescriptionTrackerSettingTab extends PluginSettingTab {
    plugin: PrescriptionTrackerPlugin;
    ...
    display(): void {
        ...
        exampleEl.createEl('h3', { text: 'Examples' });

        const exampleList = exampleEl.createEl('ul');
        exampleList.createEl('li').setText('YYYY/YYYY-MM → 2025/2025-08 (default)');
        exampleList.createEl('li').setText('YYYY/MM → 2025/08');
        exampleList.createEl('li').setText('YYYY-MM → 2025-08 (no year folder)');
        exampleList.createEl('li').setText('YYYY/MM/DD → 2025/08/03');
        exampleList.createEl('li').setText('YYYY/MMMM → 2025/August');
